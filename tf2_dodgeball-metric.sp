...
public void OnPluginStart()
{
	...
	g_hCvarDelayPreventionSpeedup = CreateConVar("tf_dodgeball_dp_speedup", "100", "How much speed (in hammer units per second) should the rocket gain (20 Refresh Rate for every 0.1 seconds) for delay prevention? Multiply by (15/352*1.609) for km/h.", FCVAR_NONE, true, 0.0, false);
		...
		if (iIndex != -1)
		{
			...
			if (GetConVarBool(g_hCvarAnnounce))
			{
				if (GetConVarBool(g_hCvarDeflectCountAnnounce))
				{
					if (iVictim == iTarget)
					{
						CPrintToChatAll("\x05%N\01 died to their rocket travelling \x05%.0f\x01 km/h with \x05%i\x01 deflections!", g_iLastDeadClient, fSpeed, iDeflections);
					}
					else
					{
						CPrintToChatAll("\x05%N\x01 died to \x05%.15N's\x01 rocket travelling \x05%.0f\x01 km/h with \x05%i\x01 deflections!", g_iLastDeadClient, iTarget, fSpeed, iDeflections);
					}
				}
				else
				{
...
public void CreateRocket(int iSpawnerEntity, int iSpawnerClass, int iTeam)
{
	...
	if (iIndex != -1)
	{
		...
		if (iEntity && IsValidEntity(iEntity))
		{
			...
			g_iRocketSpeed = RoundFloat(g_fRocketSpeed[iIndex] * 0.0685812);
